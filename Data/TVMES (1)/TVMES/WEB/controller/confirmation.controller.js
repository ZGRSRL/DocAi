sap.ui.define([
    ...
    "use strict";
    return BaseController.extend("production.controller.confirmation", {
        formatter: formatter,
        onInit: function () {
        ...
        getConfirmationList: function () {
            var locale = this.appLanguage.getSelectedKey(),
                resource = localStorage.selectedResource.split("-")[0].split(",")[1];
            var uri = "/bekorest/restapi/confirmationController/getConfirmedSfcs?site=" + this.site + "&workCenter=" + this.appWorkCenter.getSelectedKey() + "&resource=" + resource;
            var xhttpGetConfirmationList = Utils.xhttpGet(uri);
            if (xhttpGetConfirmationList.status === 200) {
                this.confirmationList = JSON
                    .parse(xhttpGetConfirmationList.responseText);
            } else {
                this.confirmationList = [];
            }
            this.confirmationListModel = new JSONModel();
            this.confirmationListModel.setSizeLimit(10000);
            this.confirmationListModel.setData(this.confirmationList.filter(x=> x.locale == "en"));
            this.getView().setModel(this.confirmationListModel,
                "confirmationListModel");
            this.getView().getModel("confirmationListModel").refresh(true);
        },
	...
	//this.getView().byId("listItemSfc").setValue(barcodeInput.getValue());
            ...
            var xhttpConfirmSfc = Utils.xhttpPost(
                "/bekorest/restapi/confirmationController/saveConfirmation?site=" + this.site + "&workCenter=" + this.appWorkCenter.getSelectedKey() + "&resource=" + resource,
                newElemJSON);
            if (!JSON.parse(xhttpConfirmSfc.response).error) {
                this.changeInfoText(JSON.parse(xhttpConfirmSfc.response).messageCode, (JSON.parse(xhttpConfirmSfc.response).errorCode === 0 ? "Accept" : "Warning"));
	   this.getConfirmationList();
	   barcodeInput.setValue("");
            } else {
                this.changeInfoText(JSON.parse(xhttpConfirmSfc.response).messageCode, "Reject");
                barcodeInput.setValue("");
            }
        }
    })
